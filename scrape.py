...
# ปรับ selector ให้ตรงกับหน้า login ถ้าจำเป็น
...
# กรอง XHR/Fetch “ที่น่าจะเป็นข้อมูล”
...
async def extract_tables_from_dom(page, tab):
    ...
    if dfs:
        ...
        return df
    # ไม่มี table → เก็บข้อความทั้งหน้าไว้ดีบัก
...
async def process_tab(ctx, page, tab):
    ...
    # Python Playwright ไม่มี page.off → ใช้ remove_listener
    try:
        page.remove_listener("response", on_response)
    except Exception:
        pass

    # ถ้ามี XHR ที่เข้าข่าย → ยิงซ้ำเก็บผล
    if hits:
        frames = []
